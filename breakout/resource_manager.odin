...
load_texture_from_file :: proc(path: string, alpha: bool) -> (Texture2D, i32) {
	...
	data := stbi.load(strings.clone_to_cstring(path), &width, &height, &nrChannels, 0)
	if data == nil {
...
load_level_from_file :: proc(path: string) -> ([dynamic][dynamic]u32, i32) {
	arr: [dynamic][dynamic]u32
	...
	for line in strings.split_lines_iterator(&it) {
		new_arr: [dynamic]u32
		append(&arr, new_arr)
		for column in strings.split(line, " ", context.allocator) {
			the_char := u32(strconv.atoi(column))
			append(&arr[index], the_char)
		}
		index += 1
	}
